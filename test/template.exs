...
defmodule App do
  @map_layout %{
		"layout" => :_layout_a9uri3fr1cCLL2kk,
  } #_MAP_LAYOUT

  @map_widget %{
		"app" => :_app_a9uri2GvxVjcyZr3,
  } #_MAP_WIDGET

  @map_page %{
		"404" => :_404_a9uriz08ygl2aBBV,
		"error" => :_error_a9uriW7z4RhmO5rP,
		"index" => :_index_a9uri9hL6A5AvVYs,
  } #_MAP_PAGE
...
defmodule LAYOUT do
	def _layout_a9uri3fr1cCLL2kk(args, cont) do
...
defmodule WIDGET do
	def _app_a9uri2GvxVjcyZr3(args) do
		"#{App.escapeHTML args[:n]}<<ICogMiA9IDwlCiAgYXJncy5uICogMgolPg==>>"
	end
end #_WIDGET

defmodule PAGE do
	def _404_a9uriz08ygl2aBBV(layout, args) do
		App.layout layout, args, %{
			body: "<<CiAgPGgxPkVycm9yIDQwNDwvaDE+CiAgPGgyPlBhZ2UgTm90IEZvdW5kITwvaDI+>>",
		}
	end
	def _error_a9uriW7z4RhmO5rP(layout, args) do
		App.layout layout, args, %{
			body: "<<CiAgPGgxPkVycm9yIA==>>#{App.escapeHTML args[:status]}<<PC9oMT4KICA8aDI+>>#{App.escapeHTML args[:error]}<<PC9oMj4=>>",
		}
	end
	def _index_a9uri9hL6A5AvVYs(layout, args) do
		App.layout layout, args, %{
